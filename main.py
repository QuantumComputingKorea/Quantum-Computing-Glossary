...
if df['영어'].str.contains(search, case=False).any() and search != '' and option == '영어':
    st._legacy_dataframe(df[df['영어'].str.contains(search, case=False)],use_container_width=True)
elif df['번역문'].str.contains(search, case=False).any() and search != '' and option == '한글':
    st._legacy_dataframe(df[df['번역문'].str.contains(search, case=False)],use_container_width=True)
elif search == '':
    st.write('')
else:
    st.write('검색어가 없습니다.')
